...
/** 顶栏区域下拉，样式与语言/主题切换器相同。onSelect(rc) 在选中其他区域时触发。 */
export function regionSwitcher(inline, currentRc, onSelect) {
  const wc = 'sw-wrap' + (inline ? ' sw-inline' : '');
  const rc = currentRc && REGION_ORDER.includes(currentRc) ? currentRc : 'ap';
  return h('div', { class: wc, id: 'swRegion' },
    h('button', {
      class: 'sw-btn', type: 'button', title: t('regionSwitchHint'),
      'aria-label': t('labelRegion'),
      onclick: (e) => { e.stopPropagation(); toggleSw('swRegion'); },
    }, faIcon('fa-earth-asia'), h('span', {}, regionLabel(rc)), faIcon('fa-chevron-down')),
    h('div', { class: 'sw-dropdown' }, ...REGION_ORDER.map((code) => h('div', {
      class: 'sw-opt' + (rc === code ? ' active' : ''),
      onclick: (e) => {
        e.stopPropagation();
        closeSw();
        if (code !== rc && typeof onSelect === 'function') onSelect(code);
      },
    }, regionLabel(code)))),
  );
}
